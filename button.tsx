...
export const InvoicePDF: React.FC<InvoicePDFProps> = ({ invoice, estimate, contractor, lineItems, companyDetails }) => {
  ...
  const title = isEstimate ? 'Estimate' : isSATaxInvoice ? 'Tax Invoice' : 'Invoice';
      ...
      <View style={styles.header}>
        ...
        <View style={styles.businessInfo}>
          ...
          <View style={styles.invoiceDetails}>
            <Text>{isEstimate ? 'Estimate' : 'Invoice'} #: {invoice.invoiceNumber || invoice.id.substring(0, 8).toUpperCase()}</Text>
            ...
            {invoice.expiresAt && <Text>Valid Until: {new Date(invoice.expiresAt).toLocaleDateString()}</Text>}
          </View>
        </View>
      </View>

      <View style={styles.clientSection}>
        <Text style={styles.sectionTitle}>{isEstimate ? 'Prepared For' : 'Bill To'}</Text>
