...
import { Plus, FileText, Banknote, Search, Download, Copy, Check, BarChart3, Trash2, ArrowUpRight, TrendingUp, MessageCircle, AlertCircle, Files } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const quotesChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'quotes', filter: `user_id=eq.${user.uid}` }, loadDashboard)
      .subscribe();

    const invoicesChannel = supabase
      .channel(`dashboard-invoices-${user.uid}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'invoices', filter: `user_id=eq.${user.uid}` }, loadDashboard)
      .subscribe();
  ...
  const handleCopyLink = (id: string) => {
    ...
    setCopiedId(id);
    toast.success('Client closing-room link copied');
    setTimeout(() => setCopiedId(null), 2000);
  };

  const handleWhatsAppShare = (quote: Quote) => {
    try {
      const share = generateWhatsAppShareLink({ ...quote, contractorBusinessName: profile?.businessName }, window.location.origin);
      trackWhatsAppShare(quote.id, 'follow_up_copilot');
  ...
  if (loading) {
    return (
      <div className="space-y-8 animate-pulse">
        <div className="h-16 bg-zinc-200 w-full max-w-xl rounded-3xl"></div>
        <div className="h-52 bg-zinc-200 rounded-3xl"></div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5">
          {[1, 2, 3, 4, 5].map(i => <div key={i} className="h-28 bg-zinc-200 rounded-3xl"></div>)}
        </div>
      </div>
    );
  }
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-zinc-500 mt-1.5 text-base font-normal">
            {user ? 'Close quotes faster, follow up warmer leads, and track invoice/payment next steps.' : 'See how SoloBid turns quote links into signed work.'}
          </p>
        </div>
        <Button asChild size="lg" className="w-full md:w-auto bg-primary hover:bg-[#03362f] text-white font-medium shadow-sm active:scale-[0.985] cursor-pointer">
          <Link to="/quotes/new"><Plus className="w-5 h-5 mr-2 stroke-[2.5]" />{user ? 'New Quote' : 'Start Free Account'}</Link>
        </Button>
      </div>
      ...
      {!user && (
        <Card className="bg-gradient-to-br from-zinc-900 to-zinc-950 text-white border-none overflow-hidden relative group rounded-3xl shadow-xl">
          <div className="absolute top-0 right-0 p-10 opacity-5 group-hover:opacity-10 transition-opacity duration-500"><BarChart3 className="w-48 h-48 rotate-12" /></div>
          <CardContent className="p-8 md:p-10 relative z-10">
            <div className="max-w-3xl">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold tracking-wider uppercase bg-teal-500/20 text-teal-300 border border-teal-500/30 mb-4">
                <TrendingUp className="w-3.5 h-3.5" /> Demo Workspace
              </span>
              <h2 className="text-2xl md:text-3xl font-semibold tracking-tight text-white mb-3">Quote closing room + follow-up copilot</h2>
              <p className="text-zinc-300 text-base mb-6 leading-relaxed max-w-2xl font-light">
                Create a quote, share a client approval link on WhatsApp, get a digital sign-off, then invoice and track EFT/payment status manually.
              </p>
              <div className="flex flex-wrap gap-3">
                <Button variant="outline" className="bg-white text-zinc-900 border-none hover:bg-zinc-100 font-medium" asChild><Link to="/">Create Free Account</Link></Button>
                <Button variant="ghost" className="text-white hover:bg-white/10 font-medium" asChild><Link to="/">Sign In</Link></Button>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-5">
        {[
          { title: 'Billed This Month', value: formatCurrency(stats.billedThisMonth), icon: Banknote, accent: 'bg-teal-50 text-teal-900 border-teal-200/50', href: '/invoices' },
          { title: 'Outstanding', value: formatCurrency(stats.outstandingBalance), icon: AlertCircle, accent: stats.outstandingBalance > 0 ? 'bg-amber-50 text-amber-800 border-amber-200/50' : 'bg-zinc-50 text-zinc-500 border-zinc-200/50', href: '/invoices' },
          { title: 'Active Quotes', value: stats.pendingCount.toString(), icon: MessageCircle, accent: 'bg-emerald-50 text-emerald-900 border-emerald-200/50', href: '/dashboard' },
          { title: 'Jobs Completed', value: stats.completedJobs.toString(), icon: Check, accent: 'bg-blue-50 text-blue-700 border-blue-100/50', href: '/invoices' },
          { title: 'Average Job Value', value: formatCurrency(stats.avgJobValue), icon: FileText, accent: 'bg-purple-50 text-purple-700 border-purple-100/50', href: '/reports' },
        ].map((stat, i) => (
          <Link key={i} to={stat.href} className={`block rounded-3xl border border-zinc-100 bg-white p-5 shadow-[0_8px_30px_rgb(0,0,0,0.015)] transition-all hover:shadow-[0_20px_40px_rgba(0,0,0,0.035)] hover:-translate-y-0.5 group cursor-pointer${i === 4 ? ' col-span-2 lg:col-span-1' : ''}`}>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-xl border ${stat.accent} flex items-center justify-center`}><stat.icon className="w-4 h-4 stroke-[2]" /></div>
              </div>
      ...
      <Card className="rounded-3xl border border-zinc-100 bg-white overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.015)]">
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 border-b border-zinc-50 bg-zinc-50/40">
          <div>
            ...
            <CardDescription className="text-zinc-500 text-xs mt-0.5">Edit, track, resend and duplicate quote-closing links.</CardDescription>
          </div>
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-64">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
              <Input type="search" placeholder="Search clients or status..." className="pl-9 h-9.5 rounded-xl border-zinc-200 bg-white shadow-sm font-normal text-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
            <Button variant="outline" size="icon" className="h-9.5 w-9.5 rounded-xl border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-500 cursor-pointer shadow-sm active:scale-95" onClick={handleExportCSV} title="Export CSV">
          ...
          {filteredQuotes.length === 0 ? (
            ...
            <div className="divide-y divide-zinc-50">
              {filteredQuotes.map((q) => {
                ...
                const badge = getStatusBadgeClassAndLabel(q.status, expired);
                return (
                  <div key={q.id} className="grid gap-4 p-5 transition-all hover:bg-zinc-50/60 md:grid-cols-[1.25fr_0.8fr_0.55fr_0.7fr] md:items-center">
                    <div>
                      <Link to={`/quotes/${q.id}`} className="font-semibold text-zinc-900 hover:text-primary transition-colors text-base line-clamp-1">{q.clientName || 'Unnamed Client'}</Link>
                      <p className="text-xs text-zinc-400 mt-0.5 line-clamp-1">{q.clientEmail || 'No email registered'}</p>
                    </div>
                    <div className="text-xs text-zinc-500 space-y-0.5">
                      <p>Created: {q.createdAt ? format(new Date(q.createdAt), 'MMM d, yyyy') : '-'}</p>
                      {q.expiresAt && <p className={expired ? 'text-red-500 font-semibold' : 'text-zinc-400'}>Expiry: {format(new Date(q.expiresAt), 'MMM d, yyyy')}</p>}
                    </div>
                    <div><span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold tracking-wider border uppercase ${badge.style}`}>{badge.label}</span></div>
                    <div className="flex flex-wrap items-center justify-start gap-1.5 md:justify-end">
                      <span className="mr-1 text-sm font-semibold text-zinc-900">{formatCurrency(q.total || 0, q.currency)}</span>
                      <Button variant="outline" size="icon" className="h-8.5 w-8.5 rounded-xl text-zinc-500 hover:text-zinc-800 bg-white border-zinc-200 shadow-sm" onClick={() => handleCopyLink(q.id)} title="Copy client closing-room link">
                        {copiedId === q.id ? <Check className="h-3.5 w-3.5 text-emerald-600" /> : <Copy className="h-3.5 w-3.5" />}
                      </Button>
                      <Button variant="outline" size="icon" className="h-8.5 w-8.5 rounded-xl border-[#25D366] bg-[#25D366] text-white hover:bg-[#1fb958] hover:border-[#1fb958] shadow-sm" onClick={() => handleWhatsAppShare(q)} title="Share on WhatsApp"><MessageCircle className="h-3.5 w-3.5" /></Button>
                      <Button variant="outline" size="icon" className="h-8.5 w-8.5 rounded-xl text-zinc-500 hover:text-primary bg-white border-zinc-200 hover:bg-teal-100/40 hover:border-primary/20 shadow-sm" onClick={() => navigate(`/quotes/${q.id}`)} title="Edit quote"><ArrowUpRight className="h-3.5 w-3.5" /></Button>
                      {user && <Button variant="outline" size="icon" className="h-8.5 w-8.5 rounded-xl text-zinc-500 hover:text-blue-600 hover:bg-blue-50 hover:border-blue-200 bg-white border-zinc-200 shadow-sm" onClick={() => handleDuplicateQuote(q)} title="Duplicate quote"><Files className="h-3.5 w-3.5" /></Button>}
                      {user && <Button variant="outline" size="icon" className="h-8.5 w-8.5 rounded-xl text-zinc-500 hover:text-red-600 hover:bg-red-50 hover:border-red-200 bg-white border-zinc-200 shadow-sm" onClick={() => setDeleteId(q.id)} title="Delete quote"><Trash2 className="h-3.5 w-3.5" /></Button>}
                    </div>
                  </div>
                );
              })}
      ...
      <ConfirmDialog
        ...
        description="Are you sure you want to delete this quote record permanently? All referenced line items, materials, and internal expense trackers will be deleted. This cannot be undone."
