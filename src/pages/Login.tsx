...
const FeatureBadge = ({ children }: { children: React.ReactNode }) => (
  <span className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.08] px-3.5 py-2 text-xs font-bold uppercase tracking-[0.14em] text-emerald-50/85 shadow-sm backdrop-blur">
    {children}
  </span>
);

const workflowSteps = [
  { title: 'Quote', desc: 'Build a professional estimate from reusable templates.' },
  { title: 'Sign', desc: 'Send one approval link your client can review.' },
  { title: 'Invoice', desc: 'Turn approved work into billing without retyping.' },
];
...
export default function Login() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#041f1b] px-4 py-5 text-white sm:px-6 lg:px-8">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_12%,_rgba(52,211,153,0.22),_transparent_30%),radial-gradient(circle_at_78%_8%,_rgba(16,185,129,0.12),_transparent_26%),linear-gradient(180deg,_rgba(255,255,255,0.06),_transparent_34%)]" />
      <div className="absolute -left-24 top-24 h-64 w-64 rounded-full bg-emerald-300/10 blur-3xl" />
      <div className="absolute -bottom-24 right-0 h-80 w-80 rounded-full bg-teal-200/10 blur-3xl" />

      <main className="relative mx-auto grid min-h-[calc(100vh-2.5rem)] w-full max-w-6xl items-center gap-9 lg:grid-cols-[1.05fr_0.95fr]">
        <section className="order-2 pb-4 text-left lg:order-1 lg:pb-0">
          <div className="mb-7 inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/[0.08] px-3 py-2 backdrop-blur-xl">
            <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-white text-emerald-950 shadow-lg shadow-black/20">
              <ReceiptText className="h-5 w-5" />
            </div>
            <div>
              <p className="text-[11px] font-black uppercase tracking-[0.24em] text-emerald-100">SoloBid</p>
              <p className="text-xs font-medium text-emerald-50/55">Quote to paid workflow</p>
            </div>
          </div>

          <h1 className="max-w-3xl text-5xl font-black leading-[0.92] tracking-[-0.07em] text-white sm:text-6xl lg:text-7xl">
            Quote it.<br />Sign it.<br />Get paid.
          </h1>

          <p className="mt-7 max-w-2xl text-base leading-8 text-emerald-50/74 sm:text-lg">
            A sharper workspace for service businesses that need professional quotes, client approval, and invoice-ready records without spreadsheet chaos.
          </p>

          <div className="mt-7 flex flex-wrap gap-3">
            <FeatureBadge>Templates</FeatureBadge>
            <FeatureBadge>Client sign-off</FeatureBadge>
            <FeatureBadge>Invoice ready</FeatureBadge>
          </div>

          <div className="mt-9 grid gap-3 sm:grid-cols-3">
            {workflowSteps.map((step, index) => (
              <div key={step.title} className="solobid-glass-dark rounded-3xl p-4">
                <div className="mb-4 flex items-center justify-between">
                  <span className="flex h-8 w-8 items-center justify-center rounded-2xl bg-white text-sm font-black text-emerald-950">{index + 1}</span>
                  {index < workflowSteps.length - 1 && <ArrowRight className="hidden h-4 w-4 text-emerald-200/50 sm:block" />}
                </div>
                <h2 className="text-sm font-black tracking-tight text-white">{step.title}</h2>
                <p className="mt-1.5 text-xs leading-5 text-emerald-50/58">{step.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="order-1 flex justify-center lg:order-2 lg:justify-end">
          <div className="w-full max-w-[430px]">
            <div className="mb-4 hidden rounded-[2rem] border border-white/10 bg-white/[0.07] p-4 shadow-2xl shadow-black/20 backdrop-blur-xl sm:block">
              <div className="rounded-[1.5rem] bg-white p-4 text-zinc-950 shadow-[0_18px_60px_rgba(0,0,0,0.18)]">
                <div className="flex items-center justify-between border-b border-zinc-100 pb-3">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-400">Current flow</p>
                    <p className="mt-1 text-sm font-bold text-zinc-950">New website quote</p>
                  </div>
                  <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">Approved</span>
                </div>
                <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                  {['Draft', 'Shared', 'Signed'].map((item, i) => (
                    <div key={item} className={`rounded-2xl border px-2 py-3 ${i === 2 ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-zinc-100 bg-zinc-50 text-zinc-500'}`}>
                      <BadgeCheck className="mx-auto mb-1.5 h-4 w-4" />
            ...
            <div className="rounded-[2rem] border border-white/12 bg-white/95 p-4 text-zinc-950 shadow-2xl shadow-black/25 backdrop-blur sm:p-5">
              <div className="mb-5 flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.22em] text-zinc-400">Access workspace</p>
                  <h2 className="mt-2 text-2xl font-black tracking-[-0.04em] text-zinc-950">
                    {isSignUp ? 'Create your account' : 'Welcome back'}
                  </h2>
                </div>
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-emerald-950 text-white">
            ...
            <p className="mt-5 flex items-center justify-center gap-2 text-xs font-medium text-emerald-50/60">
