...
export function Layout() {
  ...
  useEffect(() => {
    ...
    const advanceDate = (date: Date, frequency: string) => {
    ...
    const runRecurringInvoiceCron = async () => {
      ...
      for (const scheduleDoc of schedulesSnap.docs) {
        const schedule = { id: scheduleDoc.id, ...scheduleDoc.data() } as any;
        let issueDate = new Date(schedule.nextIssueDate);
        if (!schedule.nextIssueDate || Number.isNaN(issueDate.getTime()) || issueDate > now) continue;
        ...
        while (issueDate <= now) {
          ...
          await setDoc(doc(db, 'invoices', invoiceId), {
            ...
            invoiceNumber: `REC-${issueDate.getFullYear()}${String(issueDate.getMonth() + 1).padStart(2, '0')}-${generated + 1}`,
            clientName: schedule.clientName,
            clientEmail: schedule.clientEmail || '',
            total: schedule.total || 0,
            currency: schedule.currency || profile?.defaultCurrency || 'ZAR',
            status: 'draft',
          ...
          generated += 1;
          issueDate = advanceDate(issueDate, schedule.frequency);
        }

        await setDoc(doc(db, 'recurringInvoices', schedule.id), {
          nextIssueDate: issueDate.toISOString().slice(0, 10),
          lastProcessedAt: new Date().toISOString(),
        }, { merge: true });
      }

      if (generated > 0) {
        toast.success(`${generated} recurring invoice draft${generated === 1 ? '' : 's'} generated.`);
      }
    };

    runRecurringInvoiceCron().catch((error) => {
      console.error('Recurring invoice cron failed:', error);
    });
  }, [user, profile?.defaultCurrency]);
